...
int Authority::init()
{
    ...
    y2      = EC_KEY_new();
    y3      = EC_KEY_new();
    if (y1 == NULL || y2 == NULL || y3 == NULL || order == NULL ||
        r1 == NULL || r2 == NULL || r3 == NULL || ec_group == NULL)           { return 0; }
...
int Authority::SendComm(sgx_enclave_id_t enclave_id)
{
    ...
    if (0 == encodeBN(this->y1_sk, &y1_d, &y1len))                                          { return 0; }
    if (0 == encodeBN(this->y2_sk, &y2_d, &y2len))                                          { return 0; }
    if (0 == encodeBN(this->y3_sk, &y3_d, &y3len))                                          { return 0; }

    if (0 != send_comm(enclave_id,
                       r1_d, r1len, r2_d, r2len, r3_d, r3len,
                       y1_d, y1len, y2_d, y2len, y3_d, y3len))                              { return 0; }
...
{
    uint8_t     *r1_d, *r2_d, *r3_d, *y1_d, *y2_d, *y3_d;
    size_t      r1len, r2len, r3len, y1_dlen, y2_dlen, y3_dlen;
    Signer      *signer_tmp;
    ...
    if (0 == encodeBN(this->r2, &r2_d, &r2len))                                             { return 0; }
    if (0 == encodeBN(this->r3, &r3_d, &r3len))                                             { return 0; }
    if (0 == encodeBN(this->y1_sk, &y1_d, &y1_dlen))                                        { return 0; }
    if (0 == encodeBN(this->y2_sk, &y2_d, &y2_dlen))                                        { return 0; }
    if (0 == encodeBN(this->y3_sk, &y3_d, &y3_dlen))                                        { return 0; }
