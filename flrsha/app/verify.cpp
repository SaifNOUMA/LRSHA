...
{
    size_t        Y1len, Y2len, Y3len, R1len, R2len, R3len, ex_hash_len;
    uint8_t       ex_hash[1000], e_ptr[HASH_SIZE],
                  Y1_ptr[256], Y2_ptr[256], Y3_ptr[256],
                  R1_ptr[256], R2_ptr[256], R3_ptr[256];
    BN_CTX        *bn_ctx;
    BIGNUM        *e, *s;
    EC_POINT      *Y, *Y1, *Y2, *Y3, *R1, *R2, *R3, *Rsig, *R;
    clock_t       t0, t1;
    ...
    *req_r3 = t1 - t0;
    ...
    Y1     = EC_POINT_new(ec_group);
    Y2     = EC_POINT_new(ec_group);
    Y3     = EC_POINT_new(ec_group);
    R1     = EC_POINT_new(ec_group);
    R2     = EC_POINT_new(ec_group);
    R3     = EC_POINT_new(ec_group);
    Rsig   = EC_POINT_new(ec_group);

    if (0 == EC_POINT_oct2point(ec_group, Y1, Y1_ptr, Y1len, NULL))                             { return 0; }   /* bin2point(Y1) */
    if (0 == EC_POINT_oct2point(ec_group, Y2, Y2_ptr, Y2len, NULL))                             { return 0; }   /* bin2point(Y2) */
    if (0 == EC_POINT_oct2point(ec_group, Y3, Y3_ptr, Y3len, NULL))                             { return 0; }   /* bin2point(Y3) */

    if (0 == EC_POINT_add(ec_group, Y, Y1, Y2, NULL))                                           { return 0; }   /* Y = Y1 + Y2 mod p */
    if (0 == EC_POINT_add(ec_group, Y, Y, Y3, NULL))                                            { return 0; }   /* Y = Y  + Y3 mod p */
    ...
    *agg_R = t1 - t0;
    ...
    if (0 != ascon_hash(e_ptr, ex_hash, ex_hash_len))                                           { return 0; }   /* e = H(msg || x || j) */
